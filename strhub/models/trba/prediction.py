...
class Attention(nn.Module):
    ...
    def forward(self, batch_H, text, max_label_length=25):
        ...
        output_hiddens = batch_H.new_zeros(
            (batch_size, num_steps, self.hidden_size), dtype=torch.float
        )
        hidden = (
        ...
        if self.training:
            ...
            probs = batch_H.new_zeros(
                (batch_size, num_steps, self.num_class), dtype=torch.float
            )
