...
class ABINet(CrossEntropySystem):
        ...
        for res_list in res_lists:
            ...
            for res in res_list:
                ...
                loss += F.cross_entropy(
                    logits, targets.flatten(), ignore_index=self.pad_id
                )
            loss /= len(res_list)
    ...
    def _prepare_inputs_and_targets(self, labels):
        ...
        lengths = (
            torch.as_tensor(list(map(len, labels)), device=self.device) + 1
        )  # +1 for eos
    ...
    def training_step(self, batch, batch_idx) -> STEP_OUTPUT:
        ...
        # Pretrain submodels independently first
            ...
            a_res = self.model.alignment(
                l_res["feature"].detach(), v_res["feature"].detach()
            )
