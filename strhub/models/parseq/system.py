...
class PARSeq(CrossEntropySystem):
    ...
    def gen_tgt_perms(self, tgt):
        ...
        perms = (
            [torch.arange(max_num_chars, device=self._device)]
        ...
        if self.perm_mirrored:
            ...
            perms = (
                torch.stack([perms, comp]).transpose(0, 1).reshape(-1, max_num_chars)
            )
        # NOTE:
        ...
        if len(perms) > 1:
            perms[1, 1:] = (
                max_num_chars + 1 - torch.arange(max_num_chars + 1, device=self._device)
            )
        return perms
    ...
    def generate_attn_masks(self, perm):
        ...
        mask[torch.eye(sz, dtype=torch.bool, device=self._device)] = float(
            "-inf"
        )  # mask "self"
        query_mask = mask[1:, :-1]
    ...
    def training_step(self, batch, batch_idx) -> STEP_OUTPUT:
        ...
        for i, perm in enumerate(tgt_perms):
            ...
            loss += n * F.cross_entropy(
                logits, tgt_out.flatten(), ignore_index=self.pad_id
            )
            loss_numel += n
