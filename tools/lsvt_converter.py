...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Generate training set of LSVT " "by cropping box image."
    )
...
def process_img(args, src_image_root, dst_image_root):
    ...
    try:
        src_img = Image.open(
            osp.join(src_image_root, "train_full_images_0/{}.jpg".format(img_info))
        )
    except IOError:
        src_img = Image.open(
            osp.join(src_image_root, "train_full_images_1/{}.jpg".format(img_info))
        )
    blacklist = ["LOFTINESS*"]
...
def convert_lsvt(
    ...
    process_img_with_path = partial(
        process_img, src_image_root=src_image_root, dst_image_root=dst_image_root
    )
    tasks = []
