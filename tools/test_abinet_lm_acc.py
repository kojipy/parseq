...
class ABINetLM(ABINet):
    def _encode(self, labels):
        targets = [
            torch.arange(self.max_label_length + 1)
        ]  # dummy target. used to set pad_sequence() length
        lengths = []
        for label in labels:
        ...
        targets = pad_sequence(targets, batch_first=True, padding_value=0)[
            1:
        ]  # exclude dummy target
        lengths = torch.as_tensor(lengths, device=self.device)
        targets = (
            F.one_hot(targets, len(self.tokenizer._stoi))[
                ..., : len(self.tokenizer._stoi) - 2
            ]
            .float()
            .to(self.device)
        )
...
def main():
    ...
    parser.add_argument(
        "checkpoint",
        help="Official pretrained weights for ABINet-LV (best-train-abinet.pth)",
    )
    ...
    for name, dataloader in datamodule.test_dataloaders(test_set).items():
        ...
        results[name] = Result(
            name, total, accuracy, mean_ned, mean_conf, mean_label_length
        )

    result_groups = {"Benchmark": SceneTextDataModule.TEST_BENCHMARK}
    if args.new:
